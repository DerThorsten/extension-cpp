...
/* returns a |C_0 x c x 4 | array 
   and for junctions of size 3 the last
   entry of the last axis shall be ignored
*/
template<class float_t>
struct Cell0Extractor
{
    typedef float_t float_type;
    ...
    std::vector<at::Tensor> forward
    (
        at::Tensor features, /* |C_0| x c x eg. 128 x 128 */
        at::Tensor cell_1_masks, /* |C_0| x c x eg. 128 x 128 */
        at::Tensor cell_0_bounds /* the boundaries of each junction */
    ) 
    {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("sp_acc_av_forward", &spacc_mean_forward, "sp acc forward");
  m.def("sp_acc_av_backward", &spacc_mean_backward, "sp acc backward");
